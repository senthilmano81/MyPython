...
def resolveInvestor(investor,inv_map):
    ...
    if investor.lower() == 'accel': 
        return 'Accel Partners'

    if 'accel ' in investor.lower(): 
        return 'Accel Partners'
...
for investor in df.InvestorsName: 
    for ele in getInvestor(investor):
        ...
        if temp == 'saif partners': 
            investors.append('SAIF Partners')
        elif temp != '' and temp != 'nan' and temp != 'others' and temp != 'other': 
